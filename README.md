...
│
├── SIM ──► NanoFilt¹ ──► [Flye] ──► [Racon] (opc.) ──► [Medaka] ──► [QUAST³/BUSCO⁴ pré-polish]
│                                                                    │
│           Short reads (se houver) ─► FASTP² ─► [Polypolish] ou [NextPolish] (opc.)
│                                                                    │
│                                                                    ▼
│                                                        [QUAST³/BUSCO⁴ pós-polish]
│
└── NÃO ──► Short reads ─► FASTP² ─► [Unicycler] ──► [QUAST³/BUSCO⁴]
...
| `process_medium` | Racon, Medaka, Polypolish, NextPolish, BUSCO (+ pré/pós-polish) | `t / 2` |
...
Uma amostra só gera **um** dos três diretórios de QUAST (e, sem `--reference`, o par equivalente de BUSCO): `_prepolish/`+`_postpolish/` se veio pelo caminho Flye (denovo ou reference), ou a chamada única se veio pelo caminho Unicycler. Diretórios `busco*/` só existem quando `--reference` não foi informado.

Estrutura validada com execução real de ponta a ponta (dados sintéticos) em 18–19/07/2026, nos 3 caminhos (denovo híbrido, reference, short-only) e nos cenários com/sem `--reference`.
